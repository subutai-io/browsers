...
define(function(require, exports, module) {
  ...
  PrivateKeyController.prototype.generateKey = function(password, options) {
    ...
    this.keyring.getById(this.keyringId).generateKey({
      ...
      if (options.confirmRequired) {
        ...
        that.rejectTimer = that.porto.util.setTimeout(function() {
  ...
  PrivateKeyController.prototype.createPrivateKeyBackup = function() {
    ...
    this.pwdControl.unlockKey(primaryKey)
      ...
      .then(function() {
        ...
        that.porto.windows.openPopup(path + '?id=' + that.id, {width: 1024, height: 550, modal: false}, function(window) {
  ...
  PrivateKeyController.prototype.handlePortMessage = function(msg) {
    //console.log('privateKey.controller::' + msg.event);
    //console.log(msg);
    //console.trace();
    switch (msg.event) {
      case 'set-init-data':
      ...
      case 'generate-confirm':
        if (this.rejectTimer) {
          this.porto.util.clearTimeout(this.rejectTimer);
          this.rejectTimer = 0;
        }
        break;
